...
func TestNewRefusesAnAuroraSourceAndNamesTheOtherProvider(t *testing.T) {
	...
	_, err := rds.New(context.Background(), opts)
	require.Error(t, err)
...
// central promise. The provider publishes such a golden only when there is no
// verifier at all, which is the one path that reaches the branch side refusal.
func TestBranchingAGoldenWithNoAttestationIsRefusedWithTheMaskingCode(t *testing.T) {
	server := newFake(t, conformance.DefaultSeedSQL, "")
	p := newProvider(t, server)

	gv, err := p.RefreshGolden(context.Background(), provider.GoldenSpec{
		Version:   17,
		RulesHash: "abcd1234",
		Mask:      func(context.Context, secret.Value) error { return nil },
		// No verifier, so nothing scanned it and the version is published
		// honestly with Verified false.
	})
	require.NoError(t, err)
	require.False(t, gv.Verified)
