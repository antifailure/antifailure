...
func measureAtStorage(t *testing.T, ctx context.Context, gb int64) sizeResult {
	...
	// the provider opened while branching would fail and the branch would fail
	// with it. Zero connections is therefore measured rather than asserted
	...
	// The branch really was made against an unreachable database: proving it
	// rather than trusting the sslmode.
	...
	require.Error(t, db.Ping(),
		"the benchmark expected the branch's database to be unreachable under sslmode "+
			"require, and it answered. The zero in the connections column would then be "+
			"an assumption rather than a measurement")

	return sizeResult{storageGB: gb, actions: server.Actions(), connections: 0}
}
