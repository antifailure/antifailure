...
//
// Three of the twenty six database conformance behaviours require a specific
...
// The codes this provider has to be able to produce, plus the ones it uses for
// its own refusals.
const (
...
// ErrNotOurs reports that an instance or a snapshot carries no antifailure tag.
//
...
// tell it from an AWS failure without matching on English. What it must not do
// is borrow a code that already means something else: AF-DB-002 is "the source
// database could not be reached", and dressing this refusal in it would put a
// sentence about an unreachable host in front of somebody whose real problem is
// that they pointed the manifest at an instance they do not own. The catalog
// belongs to the engine and this module cannot add to it, so the honest answer
// is a sentinel with a sentence of its own.
var ErrNotOurs = errors.New("rds: this instance was not created by antifailure")
...
func codedWrap(code, message string, err error) *codedError {
...
//
// Asked of the target's rendered text because the target is a type this module
// cannot name. The comment at the top of this file says why that is the only
// door open.
