...
func (c *client) listInstances(ctx context.Context) ([]dbInstance, error) {
	...
	for {
		...
		if doc.Marker == "" || doc.Marker == marker {
			return out, nil
		}
		marker = doc.Marker
	}
...
func (c *client) restoreFromSnapshot(ctx context.Context, instance, snapshot, class string, tags map[string]string) (dbInstance, error) {
	...
	if err := xml.Unmarshal(body, &doc); err != nil {
		return dbInstance{}, fmt.Errorf("RestoreDBInstanceFromDBSnapshot: reading the response: %w", err)
	}
...
// addTags writes a tag map into the query API's Tags.member.N form.
//
