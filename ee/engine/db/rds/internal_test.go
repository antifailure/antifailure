...
func TestBranchNameIsDistinctForNamesThatSanitiseTheSame(t *testing.T) {
	first, err := branchName("env_a-1")
	require.NoError(t, err)
	second, err := branchName("env_a__1")
	require.NoError(t, err)
...
// Tags go on the wire in Tags.member.N order sorted by key, for the same
...
func TestAddTagsIsSortedAndSkipsEmptyValues(t *testing.T) {
	...
	require.Equal(t, "a", params.Get("Tags.member.1.Key"))
	require.Equal(t, "one", params.Get("Tags.member.1.Value"))
	require.Equal(t, "b", params.Get("Tags.member.2.Key"))
	require.Equal(t, "", params.Get("Tags.member.3.Key"),
