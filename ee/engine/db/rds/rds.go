...
type Options struct {
	...
	// SkipEngineCheck is set only by tests that have no source instance to
	// describe. It is deliberately not reachable from a manifest: the engine
	// check is what stops this provider being pointed at an Aurora cluster,
	// where the honest answer is the faster provider.
...
func New(ctx context.Context, opts Options) (*Provider, error) {
	...
	if source.Engine != enginePostgres {
		return nil, fmt.Errorf(
...
// Close releases nothing. This provider holds no long lived connection: every
...
func (p *Provider) RefreshGolden(ctx context.Context, spec provider.GoldenSpec) (provider.GoldenVersion, error) {
	...
	// are removed on every failing path, and neither is ever tagged golden, so
	// a process killed between any two of these lines leaves something the
	...
	defer func() {
		...
		defer cancel()
		// The candidate and the transit snapshot go whether or not the golden
		// was published: on success they are the scaffolding, and on failure
		// they are a copy of production nobody asked to keep.
		_ = p.destroyInstance(cleanup, candidate)
	...
	if _, err := p.api.restoreFromSnapshot(ctx, candidate, transit, p.instanceClass,
		withKind(base, kindCandidate)); err != nil {
		return provider.GoldenVersion{}, p.refreshError(err)
	}
	...
	if spec.Verify != nil {
		...
		if err != nil {
			...
			// all, so there is no artifact carrying production data that
			// failed its own scan sitting in the account waiting to be
			// branched.
	...
	case isCode(err, faultAccessDenied):
		return fmt.Errorf(
			"the credentials this provider found are not allowed to snapshot and restore "+
				...
				"rds:DeleteDBInstance, rds:DeleteDBSnapshot and the three describes: %w", err)
	}
	return err
}
...
func (p *Provider) DestroyGolden(ctx context.Context, version string) error {
	...
	for _, in := range instances {
		tags := tagMap(in.Tags)
		if tags[tagMarker] == Name && tags[tagKind] == kindBranch && tags[tagVersion] == version {
...
func (p *Provider) goldenFor(ctx context.Context, version string) (dbSnapshot, bool, error) {
	...
	// the listing.
	name := goldenPrefix + shortHash(version)
	...
	if found {
		...
		if tags[tagMarker] == Name && tags[tagKind] == kindGolden && tags[tagVersion] == version {
...
func (p *Provider) sweepCandidates(ctx context.Context) {
	...
	if snapshots, err := p.api.listSnapshots(ctx); err == nil {
		for _, s := range snapshots {
			tags := tagMap(s.Tags)
			if tags[tagMarker] != Name || tags[tagKind] != kindTransit {
				continue
			}
			if at := parseTime(tags[tagCreated]); at.IsZero() || at.Before(cutoff) {
...
func (p *Provider) Branch(ctx context.Context, version, envID string) (provider.Branch, error) {
	...
	if existing, found, err := p.api.describeInstance(ctx, name); err != nil {
		...
		if tags[tagMarker] != Name {
			return provider.Branch{}, fmt.Errorf(
				"%w: %s exists in this account and carries no antifailure tag, so this "+
					"provider will not take it over", ErrNotOurs, name)
		}
		return provider.Branch{
			EnvID:       envID,
			From:        tags[tagVersion],
			ProviderRef: existing.Identifier,
			CreatedAt:   parseTime(tags[tagCreated]),
		}, nil
	}
	...
	if joinAttestation(tagMap(snapshot.Tags)) == "" {
		// The product's central promise, enforced here rather than in a
		// checklist. A golden with no attestation was never scanned, and a
		// preview environment restored from one is a copy of production
		// nobody checked.
		return provider.Branch{}, coded(codeUnverifiedGolden, fmt.Sprintf(
			"the golden %s carries no verification attestation, so nothing has scanned "+
				"it for unmasked data and it cannot be branched", version))
	}
	...
	if _, err := p.api.restoreFromSnapshot(ctx, name, snapshot.Identifier, p.instanceClass, tags); err != nil {
		if isCode(err, faultInstanceExists) {
			// Somebody else created it between the describe above and here.
			// The instance that exists is the branch, which is what
			// idempotency by environment means.
	...
	if _, err := p.rotate(ctx, live); err != nil {
		return provider.Branch{}, err
	}
	return provider.Branch{EnvID: envID, From: version, ProviderRef: name, CreatedAt: created}, nil
}
...
func (p *Provider) countBranches(ctx context.Context) (int, error) {
	...
	for _, in := range instances {
		tags := tagMap(in.Tags)
		if tags[tagMarker] == Name && tags[tagKind] == kindBranch {
			n++
		}
	}
	return n, nil
...
func (p *Provider) destroyInstance(ctx context.Context, name string) error {
	...
	for {
		...
		if tagMap(in.Tags)[tagMarker] != Name {
			return fmt.Errorf(
				"%w: %s carries no antifailure tag, so this provider will not delete it",
				ErrNotOurs, name)
		}
...
func (p *Provider) destroySnapshot(ctx context.Context, name string) error {
	...
	if tagMap(snap.Tags)[tagMarker] != Name {
		return fmt.Errorf(
			"%w: the snapshot %s carries no antifailure tag, so this provider will not "+
				"delete it", ErrNotOurs, name)
	}
...
func (p *Provider) Health(ctx context.Context, b provider.Branch) (provider.Health, error) {
	...
	if err != nil {
		return provider.Health{Reachable: false, Detail: "the branch instance does not exist"}, nil
	}
