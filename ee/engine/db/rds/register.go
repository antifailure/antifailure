...
// Register adds the RDS provider to a registry.
//
...
// provider is a manifest saying database.provider is rds, and a build whose
...
func (Registration) Open(ctx context.Context, cfg extension.DatabaseConfig) (provider.Database, error) {
	...
	if db.Project == "" {
		return nil, fmt.Errorf(
			"database.provider is rds and database.project is empty; it is the RDS for " +
				"PostgreSQL DB INSTANCE identifier that goldens are built from, such as " +
				"acme-production. It is not a cluster identifier and not an endpoint " +
				"hostname")
	}
	...
	if region == "" {
		return nil, fmt.Errorf(
			"database.provider is rds and %s resolved to nothing. An instance is "+
	...
	if !found || key.IsZero() {
		return nil, fmt.Errorf(
			"database.provider is rds and %s resolved to nothing. It is not the source "+
				"instance's password: this provider derives a distinct master password for "+
				"every restored instance from it, so that a preview environment never "+
				"holds production's database credential. Any high entropy string will do, "+
				"and changing it changes every branch's password", variable)
	}

	return New(ctx, Options{
		SourceInstance: db.Project,
		Region:         region,
		BranchKey:      key,
		Variable:       variable,
