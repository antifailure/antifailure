...
// This is Google's explicit service-role roster, not a prefix exemption that
// could hide a customer login. The selected administrator is retained. A new
// unmanaged role that cannot be disabled causes preparation to fail closed.
...
func customerLogins(ctx context.Context, db *sql.DB) ([]string, error) {
 ...
 AND rolname NOT IN ('cloudsqladmin','cloudsqlagent','cloudsqlconnpooladmin',
...
func (p *Provider) disableInheritedLogins(ctx context.Context, connection secret.Value) error {
	...
	if err != nil {
		return fmt.Errorf("cloudsql: enumerating inherited logins: %w", err)
	}
	...
	for _, name := range names {
		if _, err := tx.ExecContext(ctx, "ALTER ROLE "+pgx.Identifier{name}.Sanitize()+" NOLOGIN PASSWORD NULL"); err != nil {
			return fmt.Errorf("cloudsql: disabling inherited login %q: %w", name, err)
		}
	}
	...
	for _, name := range append(names, admin) {
		if _, err := db.ExecContext(ctx, "SELECT pg_terminate_backend(pid) FROM pg_catalog.pg_stat_activity WHERE usename=$1 AND pid<>pg_backend_pid()", name); err != nil {
			return fmt.Errorf("cloudsql: terminating inherited sessions: %w", err)
		}
	}
	for {
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("cloudsql: inherited login sessions did not terminate: %w", ctx.Err())
