...
import { IMPERSONATION_START_PERMISSION } from '../src/admin/customers.ts'
import {
  ADMIN_PERMISSIONS,
...
/**
 * Permissions enforced somewhere other than a procedure's `adminPermission`.
 *
 * NOT AN EXEMPTION LIST, which is why it is separate from AWAITING_A_ROUTE
 * above. Everything here IS enforced; it is simply enforced by a route the
 * walk below cannot see, because the walk reads tRPC metadata and these routes
 * are not procedures. A line here asserts that the guard exists, so it carries
 * the constant the route actually checks rather than a repeated string: rename
 * the permission and both ends move together, delete the check and this import
 * stops compiling.
 *
 * The one member is impersonation. `POST /v1/admin/impersonation/start` cannot
 * be a procedure, because it ends in a Set-Cookie for the CUSTOMER's session
 * and because the operator gate refuses every procedure while a session is
 * impersonating, which would make its `end` counterpart unreachable exactly
 * when it is the only thing left to press. It calls `adminRoleHas` itself, and
 * admincustomers.test.ts asserts over real HTTP that a role without the
 * permission is refused by name.
 */
const ENFORCED_OUTSIDE_THE_ROUTER: readonly string[] = [IMPERSONATION_START_PERMISSION]
...
function declaredPermissions(): Set<string> {
  ...
  const declared = new Set<string>(ENFORCED_OUTSIDE_THE_ROUTER)
...
describe('every platform permission actually guards something', () => {
  test('the walk found routes, so the assertions below are not vacuous', () => {
    ...
    assert.ok(
      declared.size >= 15 + ENFORCED_OUTSIDE_THE_ROUTER.length,
