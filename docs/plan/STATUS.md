...
| `internal/runtime/k8s` | proven | The runtime conformance suite against a real k3s cluster: 31 behaviours passed, 1 skipped naming the capability it lacks (no ingress domain configured), 0 failed, in one process, 28 minutes, nothing left behind. Namespace per environment, five NetworkPolicies, an escape probe that refuses a cluster whose CNI does not enforce policy. Getting there took five attempts and each one found a real bug; the last was a policy that contained everything correctly and also blocked a host it was configured to allow. **THE CLUSTER RUN WAS AGAINST 2b8179f AND Up AND Down HAVE CHANGED SINCE**, to refuse a namespace this runtime did not create rather than reuse or delete it (AF-RUN-045). That change is not covered by the cluster run and is not covered by the suite either, because both new branches need a namespace that is not ours and the suite only ever makes its own. It is proven instead by unit tests in both directions, each run RED against the unguarded code first. A re-run of the full cluster suite is the thing that would retire this paragraph; it has been attempted and this row will say what happened. |
