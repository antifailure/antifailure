...
- **13.2, 13.3** are built and green, and need an Entra ID test tenant and an Okta developer account (both free) to move from `written` to `proven`. A conformance suite against a containerised Keycloak exists and is gated behind `AF_KEYCLOAK_URL`, but it has not yet been observed green: it was written against an invocation that could not work and was fixed only after the fact, which is the reason neither row claims more than `written`.
