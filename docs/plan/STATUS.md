...
| 1.8 Azure foundation (Terraform) | proven | `terraform apply` has run against the real subscription and the resource groups exist: `af-tfstate-eastus` holds the Terraform state and `af-cp-centralus` holds the control plane, both prefixed `af-` and tagged `project=antifailure`, with no subscription-scoped role assignment anywhere. **THE PLAN THAT SAID 27 TO ADD, 0 TO DESTROY WAS CLEAN AND STILL FAILED**, and that is the entry worth reading: applying it created twenty six resources and then refused the database with `ParameterOutOfRange: The value of the 'Version' should be in: []`. The empty list is literal. `az postgres flexible-server list-skus -l eastus` returns `supportedServerVersions: []` and the reason "Provisioning is restricted in this region", so PostgreSQL flexible server cannot be created in eastus on this subscription at any version in any SKU, while everything else in the stack creates there quite happily. A region therefore has THREE independent gates checked by three systems at three times: quota (which was never the constraint, 65 cores in both), Azure Policy (evaluated at write time, invisible to a plan), and regional service availability (invisible to both). The stack is in `centralus`, the only region that passes all three, at 30.47 USD a month from prices re-read for that region. `tools/azguard region` is the guard for the third gate, proved able to fail against the real API in both directions: it refuses eastus quoting Azure's own reason and allows centralus. Its unit tests cover four separate ways of not knowing, each of which must refuse rather than pass, plus a positive control so a guard that refuses everything cannot pass its own suite. Also fixed here and each found by running: `shared_access_key_enabled = false` breaks the azurerm provider's own post-create data-plane poll (`storage_use_azuread = true` is the fix); Owner on a subscription grants nothing on the storage data plane; `prevent_destroy` plus a tainted resource deadlock with an error naming the wrong one; the Key Vault name had no region in it, so a region move would have burned a global name for seven days that purge protection makes impossible to release early. Q4 is closed: the Entra app `af-infra-ci` exists with FOUR federated credentials and NO client secret or certificate at all. Four rather than two because GitHub issues OIDC subjects carrying immutable numeric organisation and repository ids, not the names every example shows, and both spellings are registered so a change in either direction does not break the job. It holds BOTH Storage Blob Data Reader and Reader on the state account, which look redundant and are not: Azure splits storage into a control plane and a data plane and a role on one grants nothing on the other, in both directions. **The claim that it holds Reader on the control plane group is FALSE and was checked rather than assumed.** `az role assignment list --scope .../af-cp-centralus` returns exactly one assignment and it is **Contributor**, to this same identity, which is what lets `cd.yml` update the container app. So the sentence about a credential being worthless to anybody who steals it does not hold: the pull request plan job and the deploy job share one identity that can change everything in the control plane's group, and a pull request can edit the workflow that uses it in the same commit. Not changed here, because splitting the identities breaks continuous deployment and that is a decision rather than a fix. **Production and alerting are APPLIED.** `app.antifailure.dev` resolves, serves a DigiCert managed certificate that `curl` verifies (`ssl_verify_result=0`), and answers `/readyz` with `ready:true` on the same image digest staging tested, promoted with the pipeline's own `deploy/cd/deploy.sh` so the migrations ran the way every future release will run them. The database is `Standard_D2ds_v4` GeneralPurpose, 64 GB, ZoneRedundant HA reporting `Healthy` with the primary in zone 1 and the standby in zone 2, geo-redundant backup Enabled and 35 days of retention, public network access Disabled. All eleven alert rules exist, are enabled, and each carries the `afcpprod-pager` action group; the pager was proved to deliver rather than assumed, with `az monitor action-group test-notifications create` returning `Status: Succeeded` for a real email. `terraform plan` against production is now empty. **THE ALERTING IS PROVEN RATHER THAN WRITTEN, AND BY ACCIDENT RATHER THAN BY TEST, WHICH IS WHY IT COUNTS.** Two rules fired on two genuinely different real failures during the stand-up and both cleared without help: `afcpprod-unreachable` at Sev0 from 20:37:37 to 22:12:38, while the certificate was issued but not yet bound and the origin reset every connection, and `afcpprod-server-errors` at Sev1 from 22:13:57 to 22:28:53, the window between the site becoming reachable and the real image being deployed, when the Terraform default image answered `/readyz` with 500. Correct severities, correct ordering, correct resolution, and nothing about it was staged. The deliberate proof exists too, `az monitor action-group test-notifications create` returning `Status: Succeeded`, and it is the weaker of the two. The availability test is confirmed probing rather than merely created: 33 to 34 runs per location from East US, West Europe and West US, with the failures during the outage window and successes since. The grant that lets `cd.yml` reach production is `azurerm_role_assignment.cd_deploys_the_group` in the stack, Contributor on the production group only, rather than an `az role assignment create` nobody could review; staging's equivalent was made by hand and is consequently absent from its state and invisible to anyone reading the stack. Which surfaced one more: `ci_principal_id` is set as a repository variable and passed by the plan job, and is NOT passed by the human who runs apply, so `azurerm_role_assignment.ci_reads_the_group` has never existed in either environment and every pull request plan has reported "1 to add" forever for a resource nobody creates. Written, wired to a value, and never applied. FOUR DEFECTS THAT ONLY AN APPLY COULD FIND, each fixed here. `output "custom_domain_verification_id"` returned a provider-sensitive value without declaring itself sensitive, which made EVERY plan of this stack exit non-zero on staging as well as production, after printing the whole plan and its own "0 to destroy" line, so it read like a plan that worked. `domain.tf` ordered the managed certificate before the custom domain binding, and Azure refuses that with `RequireCustomHostnameInEnvironment`: the hostname has to be bound first with no certificate, then the certificate issued against it, then the two bound by `az containerapp hostname bind`, which is the one thing here Terraform structurally cannot own. The certificate expiry web test was created DISABLED, because azurerm defaults `enabled` to false on a standard web test and only the availability test set it, so the enabled rule wired to the pager watched a probe that never ran: alerting that looks exactly like alerting and reports on nothing. And `high_availability[0].standby_availability_zone` was not ignored, so every future plan would have shown one in-place change forever for a zone Azure assigns and this configuration never will. Also: `backend.production.hcl` was not git-ignored while the guide said it was, which put the state storage account name one `git add` from publication. **Production and alerting were written and not applied before this.** `production.tfvars` sits beside `staging.tfvars` and explains every value that differs: a separate resource group, `high_availability` and `geo_redundant_backup` on, `GP_Standard_D2ds_v4` because HA forces a General Purpose SKU, 64 GB, 35 days of point in time recovery with the RPO written next to it, and two replicas. `modules/alerting` adds an action group and eleven rules where there were none: an availability test against `/readyz` from three locations outside the stack, 5xx, restarts, replicas below minimum, four database rules, one per job, and certificate expiry through a probe because Azure emits no metric for it. `modules/control-plane/domain.tf` owns the DNS records, the managed certificate and the custom domain binding for `app.antifailure.dev`, in the order Azure actually accepts. Measured rather than estimated: the production plan projects **353.04 USD a month** with zero UNKNOWN lines, against staging's unchanged 30.47. `GP_Standard_D2ds_v4` had no price in `infra/pricing.yaml`, and two resource types the applied staging stack has always created, `random_bytes` and `azurerm_postgresql_flexible_server_configuration`, had none either, so `tools/cost estimate` was already exiting non-zero on staging. The estimator also multiplied high availability into compute alone when Azure bills the standby's disk too. |
...
| 3.3 Masking engine | proven | All of it, not only the transforms: the 22 transforms and the key hierarchy; a catalog that reads the live schema for keys, unique constraints and generated columns; a rules model where specificity decides which rule wins rather than declaration order, and an unassigned column defaults to linking by transform; a SQL compiler; and a chunked, resumable executor that falls back to `ctid` for a table with no primary key. The package measures 84.5 percent against C.5's 100. |
...
| `af ci` | proven | run end to end; tears down on success, on failure, and on a missing runner. It now also SAYS what teardown did. The deferred call read `if downErr == nil`, so the two outcomes that matter were both silent: a teardown that could not reach the daemon printed nothing at all, which on a green run is indistinguishable from `--keep`, and one that ran and left containers behind printed only how many it removed. Each pending resource is named with what refused it, the way `af down` always has. The exit code is deliberately unchanged: it is the verdict on the change under test, and G10 is what gates a leak |
...
| GitHub App mode | proven | The App exists (`Antifailure Staging`, App ID 4756201) and is installed on `antifailure/antifailure`. `src/github/app.ts` signs App JWTs and caches installation tokens; `src/github/webhook.ts` verifies the HMAC over the raw body before parsing and writes the installation and repository rows that sign-in reads; migration 0013 confines a delivery to the account it names; `membersOf` is implemented and pages. 40 tests, three of them the delivery orderings GitHub does not promise: every handler wrote the installation row through one function that cleared `suspended_at` unconditionally, so a `repository` or `installation_repositories` delivery retried after a suspend or an uninstall put the installation back to live, and sign-in grants membership on exactly `suspended_at IS NULL`. Only the `installation` event clears it now. **PROVEN ON THE HOSTED CONTROL PLANE on 2026-08-30.** A second App exists for production, `Antifailure` (App ID 4775259, slug `antifailure`), installed on the antifailure organization as installation 157834739, with its own private key and its own webhook secret in `afcpprod-kv-centralus`; production shares nothing with staging's App, because those two secrets are what let a delivery write rows. `github_app_id` is set in `production.tfvars` and the apply was three in-place updates with nothing destroyed. What it proves, against `https://app.antifailure.dev` rather than a fixture: `/auth/github` redirects to GitHub carrying the real OAuth client id and `read:user user:email read:org`, where it carried `PLACEHOLDER-production-oauth-app-not-yet-created` an hour earlier; a delivery signed with the vault's webhook secret is answered 200 and `handled`, the same body with one character changed is 401, and a signature computed with the wrong secret is 401, where every one of the three was 503 before; and the process says so itself at start-up, `GitHub App 4775259 is configured: webhook deliveries are verified and membership can be synced`. **THE INSTALLATION WEBHOOK HAD ALREADY FAILED, WHICH IS THE FAILURE THIS ROW EXISTS TO CATCH.** GitHub sends `installation.created` once, at install time, and does not retry it. It was delivered at 00:29:51 UTC, six minutes before the App was configured, and this control plane answered **503** exactly as it is written to. So `github_installations` was empty, and empty is the state in which everybody who signs in lands with no organization and an empty screen, which is what cost this project a week on staging. Read off the App's delivery log rather than inferred: the 503 is there, and so is the redelivery at 00:39:32 answered 200 with the control plane's own `{"handled":true,"detail":"installation 157834739 for antifailure, 1 repositories"}`, which is `rememberInstallation` reporting after its INSERT rather than a status code. The delivery ids are past the range a double holds exactly, so `JSON.parse` rounds 3839993231035072512 to ...072500 and a redelivery aimed at the rounded id 404s on a delivery that never existed; the guide now says to take the id out of the raw body as text. What is NOT proven is the browser leg: nobody has yet completed a real OAuth round trip against production, so the row rests on every precondition being verified true rather than on a session existing. |
...
| 8.9 Design system | built | The web application: `console/`, a Next.js static export served by the control plane from its own origin. Tokens, type scale and mark shared with the marketing site. Loading, empty and error states for every screen. See `notes/console-web-application.md`. |
...
| `web/apps/api` | proven | 127 tests: matrix, sign-in, sessions, ingestion, membership sync, first-member ownership, break-glass. Two silent failures found by reading for the shapes this repository has shipped before. Model spend read the budget period twice, once when the request went out and once when it came back, so a completion that crossed midnight on the last day of a month was charged to a month with no budget row: the UPDATE matched nothing and the charge vanished without an error. It is charged to the month that authorised it, and a charge with nowhere to land is an error. And refusing a device login that had already been approved answered `denied: true` having denied nothing, because nothing looked at the row count |
