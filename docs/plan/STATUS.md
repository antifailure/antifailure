...
| `internal/runtime/local` | proven | 20 behaviors, plus all 32 of the runtime conformance suite against a real daemon, with no skips. The 32 predates the suite's readiness gate and is being re-run under it; the gate is not expected to change the answer here, because containment on this runtime is an internal Docker network a container is attached to at creation rather than a policy programmed asynchronously afterwards, so there is no window of the kind the Kubernetes row describes. `Egress_AllowedHostIsReached` passes under the gate, which is the behaviour that proves the others mean something. If the full re-run disagrees, this row is wrong and the run wins. |
