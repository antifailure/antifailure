...
| The console reaches a gate | proven | It never had one. `console/` is a separate npm project with its own lockfile, so it was in neither the web workspace's typecheck loop nor the runner's, and the only thing between a broken console and a release was the image build asserting `index.html` exists in a different workflow. `just typecheck` and CI now build it. |
...
What it found: forty-three defects, forty-two fixed with regression tests,
one issue open at
