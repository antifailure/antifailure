...
| 1.9 Test infrastructure and fakes | written | `engine/internal/testutil/fakes` exists, which CONTRIBUTING had claimed for some time while the directory did not. `fakes.Break` and `fakes.BreakRuntime` inject faults one broken guarantee at a time, and `fakes.Catches` maps each database fault to the conformance behaviour that must catch it. PROVEN: each fault really does corrupt the guarantee it names, against in-memory providers with controls asserting the baseline keeps all of them. NOT PROVEN: nothing here yet points the database conformance SUITE at a broken provider, so that suite is still not known to be able to fail. Lane 4 has done exactly this for the RUNTIME suite in PR #9 and the pattern is theirs to copy: no `Options.Only` is needed because the behaviours are subtests and `-run 'TestX/Behavior_Name'` already selects one, and no real service is needed either, because the property being false is arranged in the fake. What it does need is a SUBPROCESS, since a suite proving it can fail has to actually fail, and a failure inside the process asserting on it fails that process too. An earlier version of this row said a real database and an additive option were required; both were wrong. |
