...
- **13.2, 13.3** are built and green, and need an Entra ID test tenant and an Okta developer account (both free) to move from `written` to `proven`. **13.3 is now `proven` against a real Microsoft Entra ID tenant**: create and deprovision both verified in the database, over the internet, on 2026-08-28. **13.2 remains `written`**, because a real provider has authenticated a person and this code has verified that provider's signature, but no run of the single sign-on suite has been watched through to green. The Keycloak conformance suite is gated behind `AF_KEYCLOAK_URL` and was written against an invocation that could not work; it is fixed, and it has still not been seen green, which is why the word has not changed.
