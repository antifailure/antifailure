...
| 2.7 HUD | written | Model, rendering, non-TTY fallback and the Bubble Tea program. `engine/internal/hud`: reorder window that abandons a gap rather than stalling, three layouts (stacked at 80, two column at 120, three column at 160) with six golden frames committed, keyboard navigation, resize handling, and a queue that drops and counts rather than ever applying backpressure to the bus. `hud.Plain` is what a CI log gets. 55 tests, race clean, lint clean. Two property tests cover the spec's no-panic criterion, one over the model and one over the rendering that also asserts no line ever exceeds the terminal width. **NOT WIRED, and this is the honest state of it: the package has zero callers outside its own tests.** `hud.Sink` and `hud.PlainSink` make attaching it one line, but there is nothing useful to attach it to yet, because `af up` reports progress as prose rather than events and several of the event types the HUD renders are declared and emitted by nothing (lanes 9 and 11 found the same gap from the egress and telemetry side). Wiring it before the engine emits what it draws would produce a dashboard that is permanently empty and looks broken. Also outstanding: the vhs recordings for the docs page. |
...
| 8.10 Deployment | written | The control plane image exists (`deploy/docker/control-plane.Dockerfile`), is built and published by a workflow, and has been `proven` locally: built, run, migrated a fresh database, served 200. The Helm chart is `proven`: it installs on a real kind cluster in CI, and the assertion that matters passes there, `SELECT pg_has_role('af_app','antifailure_app','MEMBER')` returning true, followed by the application role reading a table and seeing no other tenant's rows. Uninstall is asserted to leave nothing behind. Its value guards are `proven` able to refuse. Terraform targets Container Apps rather than AKS: 30.47 USD a month in centralus against about 75 for an idle AKS control plane before a node runs, and it has been applied rather than only planned. Quota was never the constraint (65 cores in every candidate region, and the control plane needs none of it); the region is decided by Azure Policy and by PostgreSQL regional availability instead, which is recorded under 1.8. |
