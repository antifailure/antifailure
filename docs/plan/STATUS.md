...
| `tools/gatecheck` | proven | 29 gates in CI, every one reachable from `just gate`; two exemptions, `vuln` and `npmaudit`, both of which security.yml owns and both for the same recorded reason: their answer comes from a database outside this tree, so a green `just gate` could not promise a green CI in either direction |
