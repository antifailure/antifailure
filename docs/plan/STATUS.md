...
| 2.7 HUD | written | The model, the rendering and the non-TTY fallback. `engine/internal/hud`: a reorder window that orders by sequence and abandons a gap rather than stalling, per-pane state, a bounded tail, rune-safe truncation, and three layouts (stacked at 80, two column at 120, three column at 160) with golden frames committed for each. `hud.Plain` is what a CI log gets: one line per significant event, progress folded into a summary, warnings and errors never folded. 40 tests, race clean, lint clean, no new dependency: the renderer emits plain text, so a golden frame compares characters rather than escape sequences. Two property tests cover the spec's no-panic criterion, one over the model and one over the rendering, the second also asserting no line ever exceeds the terminal width. NOT DONE: the Bubble Tea program itself, meaning the event loop, real key handling and resize wiring (`Pane.Next`/`Prev` and focus rendering exist and are tested, but nothing reads a keystroke yet), and the vhs recordings for the docs page. |
