...
- **14.1, 14.3, 14.10** still want a cluster, which costs money for as long as it exists. The Kubernetes runtime itself is done and proven against a real k3s cluster, so this is not blocked on code. What a managed cluster would add is the three things k3d cannot show: ingress under a real domain, LoadBalancer services, and node pool behaviour.
