...
| 28 | The lifecycle diagram names emitted events. | twins | CORRECTED | The thirteen invented state labels were removed. The diagram now uses five lifecycle events emitted by the engine, rather than claiming an orchestrator state enum. |
...
| 34 | Every environment is created with a lifetime, and the sweep that removes an expired one is a command. | twins | CORRECTED | `runtime.ttl` defaults to 24h and is stamped on the environment's own resources, `af env reap` removes what has passed it, and `af env extend` is bounded by `max_ttl` measured from creation. The former claim that there is no automatic time to live was stale: what is absent is a process that runs the sweep, which a person or a schedule does. The figure's caption said "no automatic TTL" and now names the lifetime and the command. |
