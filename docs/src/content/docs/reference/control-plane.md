...
| `AF_ADMIN_DATABASE_URL` | unset | The connection string the **operator portal** uses, and a second credential rather than a second setting on the first. Its role holds `BYPASSRLS`, which is how an operator reads across tenants; the application's role must never hold it, or the isolation every other row in this table describes stops being enforced by the database. Unset means this installation has no operator portal, which is the right default for a single team, and the operator routes then say so rather than answering an empty list that reads like a platform with no customers on it. |
