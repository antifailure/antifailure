...
`/trpc` procedure plus `/health`, `/readyz`, `/v1/events` and
`/v1/auth/github-oidc`. Each operation
...
the `/auth` routes, most of `/v1`, `/metrics`, the webhooks, the model proxy,
the console's own endpoints, and the export link.
