...
| `POST /byok/anthropic/v1/messages` | engine or CLI token | The budgeted model proxy. See [Model keys](/docs/guides/model-keys). |
| `POST /byok/openai/v1/chat/completions` | engine or CLI token | The same, for OpenAI-shaped requests. |
| `GET /console/api/providers` | session cookie | Which provider keys and budgets an organization holds. Never the keys. |
| `PUT /console/api/providers/{provider}` | session cookie | Seals and stores one provider key. |
| `DELETE /console/api/providers/{provider}` | session cookie | Revokes one. |
| `PUT /console/api/providers/{provider}/budget` | session cookie | Sets the spend cap that the proxy above enforces. |
...
are asking the same organization to spend its own money.
