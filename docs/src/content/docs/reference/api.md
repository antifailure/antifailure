...
`/trpc` procedure plus `/health`, `/readyz`, `/v1/events` and the four Studio
endpoints above. Each operation has a stable `operationId`; mutation bodies and
query inputs are generated from the same Zod validators the route executes,
rather than copied into a second schema by hand. Success and refusal envelopes
are described as response schemas.
