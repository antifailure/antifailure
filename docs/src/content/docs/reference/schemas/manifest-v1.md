...
| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `comment` | boolean | no | Whether to maintain a single comment on the pull request. It is updated in place rather than appended, so a busy pull request does not accumulate twenty bot comments. Set false and neither af change nor af ci writes the report file the workflow posts, when they run inside GitHub Actions. Defaults to `true`. |
