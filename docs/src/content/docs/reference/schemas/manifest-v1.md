...
| `desktop_workflows` | list of [Desktop workflow](#desktop-workflow) | no | What the agents do in a desktop application. Written the same way a browser workflow is, as a goal and what proves it happened, and run in the same `af test` against the same environment, so a desktop result is counted and reported exactly like a browser one. Requires `desktop` to say which application they drive. Max items 200. |
...
Which application the desktop workflows drive, declared once because a manifest describes one product. It is the desktop counterpart of `base_url`: the workflows say what to do and this says what to do it to.

The application is driven through its ACCESSIBILITY TREE, the same thing a screen reader reads, which is why a desktop workflow reads like a browser one. Nothing here names a coordinate, a window position or a control's internal id.

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `application` | string | **yes** | What to launch. For `electron`, the Electron binary itself, which inside a packaged application is the executable in Contents/MacOS and in a project under development is the one in node_modules. For `macos`, the .app bundle. Relative paths are resolved against the directory holding the manifest, because the runner is a subprocess started from somewhere the manifest never mentions and a path resolved there would mean a different file. Min length 1, max length 512. |
...
One thing the agents do in a desktop application, written exactly as a browser workflow is: a sentence saying what a person would do, and what proves it happened.

WHY THIS IS ITS OWN LIST rather than a `surface` key on `workflows`, which is the same reason terminal workflows are their own list. A browser workflow needs a persona to sign in as and a path to start at; a desktop application has neither, because there is no address bar to open and no cookie to set, and signing in is itself a workflow that types into the fields the application shows. Putting both in one entry would mean half of every entry's keys are refused by the other half's surface. The list a workflow is written in says which surface it drives, and that is a fact a person can see.

Names are unique across all three lists, because a name is what `--only` selects and what the report prints.

| Field | Type | Required | Notes |
| --- | --- | --- | --- |
| `answers` | object | no | What to type into fields this workflow names, keyed by any part of a field's accessible name and matched without regard to case.  A table, not a script. It says what an answer IS, never what order to do things in or which control to press, so a workflow with answers is still a sentence with some of its nouns pinned down. The planner still decides everything else. Max properties 50. |
| `budget` | object | no | What this workflow may spend. Both a duration and a number of steps, unlike a terminal workflow, because a desktop workflow is planned rather than written down: something decides what to press next, and a plan that never finishes has to be stopped by a count as well as by a clock. |
| `description` | string | **yes** | What a person would do and what proves it happened, in sentences. It is also read: a control whose whole visible label appears here is one the agent is willing to press, which is how a screen nothing else recognises is still driven. Min length 10, max length 4000. |
| `expect` | list of string | **yes** | What the screen must show, written as sentences about what a person would read. Judged against the accessible text of the window: the headings, labels and static text a screen reader would announce.  It is NOT judged against what the agent typed. A field's own value is left out of that text deliberately, because an expectation a workflow can satisfy by filling a box with its own answer is a check that cannot say no.  At least one is required, as for a terminal workflow: a desktop workflow with nothing to expect can only ever report that nothing confirmed or contradicted it, which is blocked, so declaring one writes a workflow that cannot pass.  A sentence in double quotes is required on screen character for character. Min items 1, max items 50. |
