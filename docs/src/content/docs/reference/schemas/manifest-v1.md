...
| `plan_regression` | `ignore`, `warn`, `fail` | no | A query plan that got worse in one of three ways: a table is now read end to end, an index is no longer used, or the planner's estimate grew. Defaults to `warn`. |
