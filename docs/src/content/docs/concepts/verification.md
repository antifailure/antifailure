...
This is enforced in every provider rather than in a checklist. It is the
product's central promise: an environment cannot contain unmasked production
data, because the only thing an environment can branch is a golden, and a
golden is not a golden until the scan passed.
