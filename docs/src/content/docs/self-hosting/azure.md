...
**Terraform generates two of them and reads three.** The operator database
...
minted on somebody else's service, so Terraform reads them out of the vault
rather than writing them. That is the same treatment the GitHub App's private
key gets and it is deliberate: a resource that manages a value it cannot produce
is a resource that will one day set it to the empty string.

So the order is: **put the secret in the vault, then set the switch.** A plan
with the switch on and no secret in the vault fails on the data source, naming
the secret it could not find, which is a truthful refusal rather than a deploy
that comes up broken.
