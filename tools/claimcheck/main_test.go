...
// Generated Go carries literals somebody wrote somewhere else.
//
// engine/internal/docs/pages.gen.go carries the documentation site itself,
// page by page, so every address any page mentions arrives in Go source
// looking like an address the engine hands a user. The first one to arrive was
// /docs/llms-full.txt, which docs/src/pages/llms-full.txt.ts serves and which
// is not a page under src/content/docs, so this check called a live address
// dead. The claim is real prose in index.md and `just links` is what checks
// it.
func TestAURLInGeneratedGoIsCheckedWhereItWasAuthored(t *testing.T) {
	root := t.TempDir()
	src := "// Code generated by tools/docsembed. DO NOT EDIT.\n\npackage p\n\n" +
		"const X = \"https://antifailure.dev/docs/nope/missing\"\n"
	if err := os.WriteFile(filepath.Join(root, "x.gen.go"), []byte(src), 0o600); err != nil {
		t.Fatal(err)
	}
	var out strings.Builder
	if err := checkDocsURLs(root, trackedSet(), &out); err != nil {
		t.Fatalf("a generated file's literal must not fail the build: %v\n%s", err, out.String())
	}
}

// The marker only counts before the package clause, so a file that merely
// quotes the sentence, such as the generator that writes it, is still checked.
func TestQuotingTheGeneratedMarkerDoesNotExcuseAFile(t *testing.T) {
	root := t.TempDir()
	src := "package p\n\n" +
		"// Code generated by tools/docsembed. DO NOT EDIT.\n" +
		"const X = \"https://antifailure.dev/docs/nope/missing\"\n"
	if err := os.WriteFile(filepath.Join(root, "x.go"), []byte(src), 0o600); err != nil {
		t.Fatal(err)
	}
	var out strings.Builder
	if err := checkDocsURLs(root, trackedSet(), &out); err == nil {
		t.Fatal("a hand written file quoting the marker must still be checked")
	}
}
