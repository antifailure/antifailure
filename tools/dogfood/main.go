...
func (r *runner) readEvents(run *Run) {
	dir := filepath.Join(r.root, ".antifailure", "events")
...
// labelEnv is the label the engine puts an environment identifier in.
//
// Written out rather than imported: internal/dockerutil is internal to the
// engine module, and a constant copied with its source named is a smaller
// price than widening that package's surface for one string.
const labelEnv = "dev.antifailure.env"
