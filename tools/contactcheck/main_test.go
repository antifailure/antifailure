...
// Generated Go carries addresses somebody wrote somewhere else.
//
// engine/internal/docs/pages.gen.go packages the documentation site page by
// page, so an example address inside a guide arrived here as an address this
// project publishes and cannot answer, reported against a generated filename
// nobody can fix. The guide itself is scanned, which is where the sentence
// lives and where somebody can change it.
func TestGeneratedGoIsScannedWhereItWasAuthored(t *testing.T) {
	root := t.TempDir()
	writeFile(t, filepath.Join(root, "engine", "internal", "docs", "pages.gen.go"),
		"// Code generated by tools/docsembed. DO NOT EDIT.\n\npackage docs\n\n"+
			"var Pages = map[string]string{\"guides/x.md\": \"write to someone@gmail.com\"}\n")
	writeFile(t, filepath.Join(root, "engine", "internal", "real", "real.go"),
		"package real\n\nconst c = \"write to someone@gmail.com\"\n")
	...
	if scanned["engine/internal/docs/pages.gen.go"] {
		t.Error("the packaged documentation was scanned as if the engine published it")
	}
	if !scanned["engine/internal/real/real.go"] {
		t.Error("hand written Go was not scanned, so this test proved nothing about the " +
			"skip: it would pass with every Go file skipped")
	}
}
