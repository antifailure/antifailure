...
// Generated Go carries codes somebody wrote somewhere else.
//
// engine/internal/docs/pages.gen.go packages the documentation site page by
// page, so the error reference LISTING AF-CPL-003 as reserved arrived here as
// evidence that something returns it, and this gate reported a planned code
// that ships. That is a real defect class and this was not one of them.
func TestACodeInGeneratedGoIsNotACodeSomethingReturns(t *testing.T) {
	root := t.TempDir()
	write(t, filepath.Join(root, "engine", "internal", "docs", "pages.gen.go"),
		"// Code generated by tools/docsembed. DO NOT EDIT.\n\npackage docs\n\n"+
			"var Pages = map[string]string{\"reference/errors.md\": \"AF-EE-004 is reserved.\"}\n")
	write(t, filepath.Join(root, "engine", "internal", "real", "real.go"),
		"package real\n\nconst c = \"AF-DB-001\"\n")
	...
	if used["AFEE004"] {
		t.Error("a code quoted by the packaged documentation was counted as one the " +
			"engine returns")
	}
	if !used["AFDB001"] {
		t.Error("a code in hand written Go was not counted, so this test proved nothing " +
			"about the skip: it would pass with every file skipped")
	}
}
