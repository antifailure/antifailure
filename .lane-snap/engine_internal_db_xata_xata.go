...
//
// Of the thirteen managed Postgres vendors this lane read, Xata is the only one
// whose branching is really branching. Its own reference says a child branch
// copies the parent's schema and data using a copy on write storage snapshot
// and completes in seconds even for terabyte scale databases, on OpenEBS
// volumes under CloudNativePG. Every other vendor on that row calls the same
// operation a fork and restores a backup, where the clock grows with the data.
// The comparison is in docs/src/content/docs/providers/managed-postgres.md,
// with the page and the date behind every verdict.
//
// So this provider declares Caps.CopyOnWrite TRUE, and the declaration is the
// interesting part of the file rather than a line in a struct.
//
// WHAT THAT DECLARATION IS WORTH TODAY, STATED HERE RATHER THAN IN A REPORT
// SOMEBODY WOULD HAVE TO FIND.
//
// engine/conformance/cow.go can falsify a copy on write claim. It builds a
// small golden and a large one, times several branches of each, and refuses the
// declaration when the larger one costs more than the machine's noise can
...
// against the harness this provider is tested with. The test here is a fake
// control plane over a real local Postgres, which is the shape the plan
// prescribes and which proves the provider's logic, its request shapes and its
// error mapping. It cannot exhibit copy on write, because the only way a fake
// over one local Postgres can hand back a branch carrying the golden's data is
// to copy it, and a copy is what the behaviour refuses. So the alternatives
// were a declaration this repository knows is false, a behaviour switched off,
// or a truthful declaration that is not yet proved. This is the third, and the
// suite has no way to say so yet: a lane is building the third verdict for it.
//
// What is proved: the provider's logic, its request shapes and its error
// mapping, against a fake control plane over a real local Postgres. What is NOT
// proved: that Xata accepts those requests, and any wall clock number. No
// account was created and no branch was made on Xata.
//
// THE MODEL, which is Neon's and is not a coincidence.
//
// A Xata project holds the production database on its parent branch. A golden
// is a copy on write branch of that parent, masked and verified in place and
// then published by a rename. An environment's database is a copy on write
// branch of the golden. Nothing is ever copied by this provider, which is the
// whole point of choosing a vendor whose branches share storage.
//
...
// because Xata publishes no endpoint that returns a branch to another branch's
// state: implementing reset as a delete and a recreate would hand back a
// different branch on a different connection string, which is not what the
// interface's caller asked for.
package xata
...
//
// It is also the only place it CAN go. Xata's branch object has one free text
// field, description, capped at fifty characters, which is enough for a golden
// version identifier and nothing else.
...
func (p *Provider) Capabilities() provider.Caps {
	return provider.Caps{
		Branching: true,
		// FALSE, and it is the honest answer rather than a gap. Xata publishes
		// create, list, get, rename and delete for a branch and nothing that
		// returns a branch to another branch's state. Reset implemented as a
		// delete and a recreate would hand back a different branch on a
		// different connection string, which is not what the caller asked for,
		// and the suite skips the behaviour naming the missing capability
		...
		CopyOnWrite: true,
		// Xata can create branches with data masking of its own. This provider
		// does not use it: the engine's rules are the single implementation of
		// masking, and a provider's masking is a claim where verification is a
		// check.
		...
		Subsetting: false,
		// The credentials endpoint returns one connection string. Xata's
		// reference documents no pooled variant, so declaring pooled endpoints
		// would mean handing the same direct string back twice under two names.
		...
		// Xata runs vanilla Postgres and its images are chosen per branch. The
		// majors listed are the ones this provider has a reason to accept; a
		// candidate inherits the parent's image, so the real answer is whatever
		// the project runs and a manifest asking for something else has to be
		// refused rather than silently given the parent's version.
		SupportedVersions: []int{15, 16, 17, 18},
	}
}
...
// request nobody wrote. Xata documents no pattern for a branch name, so this is
// the conservative reading rather than a transcription of a published rule, and
// saying so is the difference between a decision and a claim.
