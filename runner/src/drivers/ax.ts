...
export function snapshotFrom(
  ...
  for (const { node, parent } of walk(root)) {
    ...
    if (TEXT_ROLES.has(role)) {
      ...
      // A FIELD'S OWN VALUE IS NOT EVIDENCE, AND THIS IS THE LINE THAT SAYS SO.
      //
      // Expectations are judged against this string. A field's value is what
      // the AGENT typed into it a moment ago, so including it means a workflow
      // that fills "Email address" with "Welcome back" and then expects
      // "Welcome back" passes without the application having done anything at
      // all. That is a check that cannot say no, which is the one defect this
      // repository refuses to ship, and it is the accessibility tree's version
      // of a pseudo terminal echoing what was typed.
      //
      // The browser surface never had this hole and the asymmetry is the proof
      // that this was mine to close rather than an inherent property of the
      // idea: `body.innerText` does not include the value of a form control,
      // so the web driver has always judged expectations against what the page
      // RENDERED. Excluding the field roles here is what makes the desktop
      // snapshot mean the same thing.
      //
      // Values are still read for every other role, and they have to be: on
      // macOS an AXStaticText carries its words in AXValue rather than in
      // AXTitle, so dropping values wholesale would empty the text of a native
      // screen. A page that echoes an order number back at you is still
      // evidence, because it arrives as static text rather than as the
      // contents of the box you typed it into.
