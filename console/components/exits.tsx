...
/**
 * The four exits, as components rather than as a page.
 *
 * Signing a session out, taking a copy of everything, deleting the
 * organization and closing your own account. They were written inside
 * Settings, which is the screen they belong on for a customer whose plan is
 * healthy, and they are here because there is a second screen that needs the
 * same four and must not have a second implementation of them.
 *
 * That second screen is the lapsed one. On the hosted service these four
 * permissions are exempt from the plan gate, for the reason written next to
 * HOSTED_GATE_EXEMPT in web/apps/api/src/hosted.ts: a plan gate may restrict
 * what the product does for a customer and may never restrict their ability to
 * leave, to retrieve what is theirs, or to secure their account. A copy of
 * these components would be a copy that drifts, and the direction it would
 * drift is the one where the lapsed customer gets the older, worse version of
 * the control they are entitled to.
 *
 * What is deliberately NOT here: renaming the organization, the billing
 * contact, and the invitations. Those are the product doing work, they stay
 * gated, and a lapsed screen that rendered them would be a screen of controls
...
export function saidWrong(err: unknown): string {
...
export function Said({ tone, children }: { tone: "ok" | "bad"; children: React.ReactNode }) {
  return (
    <p
...
export function Sessions({ csrf }: { csrf: string }) {
...
export function ExportCard({ csrf, slug }: { csrf: string; slug: string }) {
...
/**
 * What the deletion card needs about the organization, and no more.
 *
 * It used to take the whole `Settings` object, which is the response of a
 * route under `environments.view`. That permission is refused on a lapsed
 * hosted plan, so a component typed against that response could only ever be
 * rendered on a screen the lapsed customer cannot reach. Narrowing the prop to
 * the four facts it actually reads is what lets the same card be fed by the
 * exempt read instead.
 *
 * `counts` is OPTIONAL and that is the whole point of it being written down
 * here. The read that carries them is gated, so on the lapsed screen they are
 * genuinely absent, and the confirmation says what is deleted without numbers
 * rather than saying zero. A missing answer rendered as "0 repositories" would
 * tell somebody about to delete their organization that there is nothing in
 * it, which is the one sentence on this screen that must never be guessed.
 */
export interface OrgFacts {
  ...
  counts?: { members: number; repositories: number } | null;
}
...
export function Deleting({
  ...
  /**
   * The deletion as this card last saw it, which is not always what the
   * parent's read says.
   *
   * `deletion.request` answers with the fresh view AND with a download link
   * that is generated once and stored nowhere, because after the purge there
   * is no membership left to authorise a download. This card used to take that
   * link into state and then immediately call the parent's reload, which puts
   * the parent read back into its loading branch, unmounts this card, and
   * throws the link away before anybody could copy it. The copy beside it says
   * "it is shown once", and it was: for about one frame.
   *
   * So the response is rendered rather than re-fetched. Nothing unmounts, and
   * the link stays on screen until the person leaves the page.
   */
  const [requested, setRequested] = useState<Deletion | null>(null);
  const deletion = requested ?? org.deletion;
          ...
          {exportUrl ? (
            <div className="rounded-md border border-rule bg-paper px-3.5 py-3">
              ...
              <p className="mt-1 max-w-[62ch] text-[12px] leading-5 text-muted">
                ...
                once, and it works for {org.exportRetentionDays} days.
              </p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={async () => {
                  ...
                  setError(null);
                  try {
                    // Same reason as above: reloading here would unmount the
                    // card and take the one-time link with it.
                    const next = await mutate<{ moved: boolean; deletion: Deletion }>(
                      "deletion.advance",
                      {},
                      csrf,
                    );
                    setRequested(next.deletion);
                  } catch (err) {
        ...
        <Confirm
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              setCancelling(false);
              // The only place a reload is still right: the deletion is off,
              // there is no one-time link left to lose, and the parent read is
              // the truth again.
              setRequested(null);
              setExportUrl(null);
              onChanged();
    ...
    >
      ...
      <Confirm
        ...
        onConfirm={async () => {
          ...
          setError(null);
          try {
            const started = await mutate<{ deletion: Deletion; exportUrl: string }>(
              "deletion.request",
              { confirm: org.slug, reason: reason.trim() || undefined },
              csrf,
            );
            setExportUrl(started.exportUrl);
            setRequested(started.deletion);
            setAsking(false);
          } catch (err) {
            setError(saidWrong(err));
          } finally {
      ...
      >
        <p>
          {org.counts ? (
            <>
              This deletes {count(org.counts.repositories, "repository", "repositories")},{" "}
              {count(org.counts.members, "person", "people")}, every environment, every run and
              verdict, the masking rules, the egress policy, the billing history and the audit log.
            </>
          ) : (
            <>
              This deletes every repository, every membership, every environment, every run and
              verdict, the masking rules, the egress policy, the billing history and the audit log.
            </>
          )}
        </p>
 ...
 */
export function CloseAccount({ csrf, label }: { csrf: string; label: string }) {
  const [asking, setAsking] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
