...
Read docs/guides/aws.md for the surface and the measurements behind it. An
egress rule selects mode emulate and emulator aws; the Docker runtime starts
the registered image and routes matching hosts through the sidecar. This
change supplies the built-in registration, generated notices, published
surface and SDK suite. Separate runtime tests prove application routing,
containment and teardown. A test fails when the guide and code disagree.
