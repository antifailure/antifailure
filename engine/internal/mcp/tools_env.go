...
// readEnvStatus reports what is running for the branch this server is on.
type readEnvStatus func(ctx context.Context) (*env.Result, error)
...
type branchStatusDoc struct {
	...
	Services []envServiceDoc `json:"services"`
...
func newInspectEnvironmentsTool(
	p *Project, status readEnvStatus, inventory readInventory, pull pullEnvironmentRecord,
) *Tool {
	return &Tool{
...
func inspectEnvironments(
	...
	status readEnvStatus, inventory readInventory, pull pullEnvironmentRecord,
	args map[string]any,
) (any, *Fault) {
...
func describeBranchStatus(res *env.Result) branchStatusDoc {
	doc := branchStatusDoc{Services: []envServiceDoc{}}
	if res == nil {
		return doc
	}
...
func newExtendEnvironmentTool(p *Project, extend extendEnvironment) *Tool {
	return &Tool{
		...
		Handler: func(ctx context.Context, _ *Call, args map[string]any) (any, *Fault) {
			...
			hours, fault := floatField(args, "hours")
			if fault != nil {
				return nil, fault
			}
...
// floatField reads a number that validation already bounded.
func floatField(args map[string]any, field string) (float64, *Fault) {
