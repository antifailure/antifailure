...
// A repository is almost never cloned into a directory named after its
// package, and the Dockerfile analyzer has nothing but the directory to name a
// service after. Before this was folded, that disagreement produced one
// application as two services on one port, af init refused to write the
// manifest, and the user was told to fix a line in a file that did not exist.
func TestRun_ADockerfileDoesNotBecomeASecondServiceBesideThePackage(t *testing.T) {
	t.Parallel()
	res := run(t, "checkout-dir", map[string]string{
		"package.json": `{"name":"@acme/storefront","scripts":{"start":"next start"},"dependencies":{"next":"15.0.0"}}`,
		"Dockerfile":   "FROM node:22\nEXPOSE 3000\nCMD [\"node\",\"server.js\"]\n",
	})
	require.Len(t, res.Draft.Services, 1, "one application is one service whatever the directory is called")
	svc := serviceNamed(t, res.Draft, "storefront")
	require.Equal(t, 3000, svc.Port)
	require.Equal(t, schema.BuildDockerfile, svc.Build.Strategy, "the Dockerfile still builds it")
	require.Equal(t, "node server.js", svc.Command)
}

// The fold keys on the directory, so a Dockerfile in one package must not be
// pulled onto a package beside it.
func TestRun_ADockerfileFoldsOnlyIntoItsOwnDirectory(t *testing.T) {
	t.Parallel()
	res := run(t, "repo", map[string]string{
		"api/package.json": `{"name":"@acme/api","dependencies":{"express":"4.0.0"},"scripts":{"start":"node server.js"}}`,
		"api/Dockerfile":   "FROM node:22\nEXPOSE 4000\nCMD [\"node\",\"server.js\"]\n",
		"web/package.json": `{"name":"@acme/web","dependencies":{"next":"15.0.0"},"scripts":{"start":"next start"}}`,
	})
	api := serviceNamed(t, res.Draft, "api")
	require.Equal(t, 4000, api.Port)
	require.NotNil(t, api.Build, "the Dockerfile beside it builds it")
	web := serviceNamed(t, res.Draft, "web")
	require.Nil(t, web.Build, "a Dockerfile in another package builds nothing here")
}

// Kinds are part of the identity. A Procfile worker in the same directory as
// the web package is a second service, not evidence about the first.
func TestRun_AWorkerIsNotFoldedIntoTheWebServiceThatShipsWithIt(t *testing.T) {
	t.Parallel()
	res := run(t, "checkout-dir", map[string]string{
		"package.json": `{"name":"@acme/storefront","dependencies":{"next":"15.0.0","bullmq":"5.0.0"},"scripts":{"start":"next start","worker":"node worker.js"}}`,
		"Dockerfile":   "FROM node:22\nEXPOSE 3000\nCMD [\"node\",\"server.js\"]\n",
	})
	require.Equal(t, schema.ServiceWeb, serviceNamed(t, res.Draft, "storefront").Kind)
	require.Equal(t, schema.ServiceWorker, serviceNamed(t, res.Draft, "storefront-worker").Kind)
}

// The pair above and TestRun_DockerfileOutranksAFrameworkDefault are the same
// repository under two checkout names, so they have to detect the same thing.
// Asserting it directly is what stops the fold from quietly resolving evidence
// differently from the path that does not need it.
func TestRun_DetectionDoesNotDependOnTheCheckoutDirectoryName(t *testing.T) {
	t.Parallel()
	files := func(pkg string) map[string]string {
		return map[string]string{
			"package.json": `{"name":"` + pkg + `","scripts":{"start":"next start"},"dependencies":{"next":"15.0.0"}}`,
			"Dockerfile":   "FROM node:22\nEXPOSE 8080\nCMD [\"node\",\"server.js\"]\n",
		}
	}
	agreed := serviceNamed(t, run(t, "app", files("app")).Draft, "app")
	differing := serviceNamed(t, run(t, "some-clone", files("app")).Draft, "app")
	require.Equal(t, agreed.Port, differing.Port)
	require.Equal(t, agreed.Command, differing.Command)
	require.Equal(t, agreed.Build, differing.Build)
}
