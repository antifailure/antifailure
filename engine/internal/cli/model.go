...
until they disagree, and then the only useful sentence is which one won.`),
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// No headings, because the left column is the label. Neither
			// column flexes: the values here are endpoints and explanations
			// that mean nothing with their tails cut off, so a terminal too
			// narrow to hold them stacks the rows instead of truncating.
			e.Out.Table([]Column{Col(""), Col("")}, rows)
