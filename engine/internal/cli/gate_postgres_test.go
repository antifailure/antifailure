...
func TestGatePostgres_AFindingFollowsTheMeasurementNotTheStatement(t *testing.T) {
	...
	// sampled.
	//
	// The lock_timeout is here because the DDL lint grew a no_lock_timeout rule
	// after this test was written, and it was right to fire: an ALTER TABLE with
	// no lock_timeout does queue behind whatever holds the table. Without it the
	// relaxed half below saw a warn finding that had nothing to do with lock
	// hold times and read as the gate inventing one.
	r := rehearseForGate(t, map[string]string{
		"001_fast.sql": "SET lock_timeout = '3s';\nALTER TABLE orders ADD COLUMN note text;\n",
	}, 200, "fastmigration")
	require.False(t, r.Failed, r.Error)
