...
// is CREATE DATABASE ... TEMPLATE, which copies the files. So the branch this
// fake makes is a copy, engine/conformance/cow.go's copy on write behaviour
// would refuse the provider's truthful CopyOnWrite: true declaration when run
// against it, and TestConformanceAgainstTheFake below runs it anyway, behind an
// environment variable, so that the refusal is a thing somebody can read rather
// than a thing this file asserts about itself.
...
func (f *fakeXata) writeError(w http.ResponseWriter, status int, message string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]string{"id": "fake_error", "message": message})
}
...
func (f *fakeXata) list(w http.ResponseWriter) {
	f.mu.Lock()
	out := make([]*fakeBranch, 0, len(f.branches))
	for _, b := range f.branches {
		out = append(out, b)
	}
	f.mu.Unlock()
...
func (f *fakeXata) create(w http.ResponseWriter, body map[string]any) {
	...
	if len(description) > 50 {
		f.writeError(w, http.StatusBadRequest, "description is capped at 50 characters")
		return
	}
	...
	for _, b := range f.branches {
		if b.Name == name {
			f.mu.Unlock()
			f.writeError(w, http.StatusPreconditionFailed, "a branch named "+name+" already exists")
			return
		}
	}
	...
	if ok {
		for _, other := range f.branches {
			if other.ParentID != nil && *other.ParentID == id {
				f.mu.Unlock()
				f.writeError(w, http.StatusPreconditionFailed,
