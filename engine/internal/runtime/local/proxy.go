...
func (r *Runtime) startProxy(
	...
	if existing, err := r.cli.ContainerInspect(ctx, name, client.ContainerInspectOptions{}); err == nil {
		if existing.Container.Config == nil || !dockerutil.IsOurs(existing.Container.Config.Labels) || existing.Container.Config.Labels[dockerutil.LabelEnv] != envID {
			return "", aferrors.Coded(aferrors.AFRUN040, "detail", "the existing proxy does not belong to this environment")
		}
		if existing.Container.State != nil && existing.Container.State.Running && r.runsImage(ctx, existing.Container, proxyimage.Tag()) && existing.Container.Config.Labels[configurationLabel] == fingerprint {
