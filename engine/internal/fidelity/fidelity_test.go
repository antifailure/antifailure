...
func TestTraffic(t *testing.T) {
	...
	t.Run("traces are production's traffic just as an access log is", func(t *testing.T) {
		// This is the case that used to be reported as the engine's default
		// shape. The arm above tested the manifest's declared source against
		// access_log, which was every connected source when it was written,
		// so making OpenTelemetry real turned a correct report into a wrong
	...
	t.Run("a source that produced nothing is absent with the engine's own reason", func(t *testing.T) {
		// The reason wins over everything, including a source that is
		// connected and configured. An export of nothing but client spans
		// parses fine and yields no requests, and the report has to say that
		// rather than fall back to a shape it did not read.
		obs := full()
		obs.Manifest.Load = &schema.Load{Enabled: true, Source: schema.LoadOTel}
		obs.TrafficReason = "no server spans in ops/traces.jsonl; 412 client spans were skipped"
		c := componentState(t, fidelity.Build(obs), schema.FidelityTraffic, "endpoint mix")
		require.Equal(t, fidelity.Absent, c.State)
