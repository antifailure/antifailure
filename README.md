...
The installer downloads the release for your platform, checks it against the published checksum, and puts `af` and its runner under `~/.antifailure`. It is POSIX sh rather than bash, so it works in an Alpine container as well as on a laptop. Verification fails closed: a checksum file that will not download, an archive not named inside one, a machine with no `shasum`, `sha256sum` or `openssl`, and an archive missing a file it promised are all refused with a message naming what was missing, rather than installed with a warning nobody reads.
