...
export const CONTROL_PLANE_ROUTES = {
  ...
  "auth.github": {
    ...
    probeEffect: "writes",
    probeReason:
      "auth/signin.ts beginSignIn inserts one row into oauth_states unconditionally, and nothing in front of the handler refuses a request that carries no credential. A probe therefore creates one state row, exactly as a person who clicks the button and then closes the tab does. There is no request that proves this route is present without doing that, so it is declared rather than hidden. SAY WHAT THE ROW COSTS EXACTLY, because the imprecise version of this sentence said the row was 'expiring' and invited a reader to assume it cleans itself up. It did not clean itself up: completeSignIn deletes a state ON REDEMPTION and, until sweepOAuthStates, nothing swept the table, so an unredeemed row was unusable after OAUTH_STATE_TTL_MS and stayed in the table for good. The control plane now sweeps states a day past expiry from the same housekeeping interval as sessions, devices and sign-in links, so the row this probe leaves is reclaimed rather than permanent. Read that as a claim about the DEPLOYED control plane rather than about main: the sweep ships on the tag clock, so a probe sent at an origin older than it still leaves a row that nothing will remove.",
  },
