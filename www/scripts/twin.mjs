/**
 * Turns one rendered page into its markdown twin, without touching the disk.
 *
 * Moved out of markdown-twins.mjs, which runs its whole build step the moment it
 * is loaded, so that a test can hand this a page and read the twin it produces.
 * markdown-twins.mjs still owns walking out/ and writing the files.
 */
...
/**
 * The twin of one page, or null for a page that gets none: a page that is not
 * itself indexable, or one with no content to extract.
 *
 * `rel` is the page's path under out/, which the twin's first line names.
 */
export function renderTwin(html, rel) {
  ...
  const markdown = [
    `<!-- Generated from ${rel} at build time. Do not edit. -->`,
    "",
